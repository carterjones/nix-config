...
case "${OS}" in
Arch)         docker run -v $(pwd):/nix-config carterjones/arch         /bin/bash -c "cd /nix-config && ./install && ./install";;
Manjaro)      docker run -v $(pwd):/nix-config carterjones/manjaro      /bin/bash -c "cd /nix-config && ./install && ./install";;
